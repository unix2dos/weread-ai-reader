...
AI 摘要窗口展示极简首屏：精读/快读/跳读结论、掌握价值分、最多 3 个最需要掌握点、最多 2 个带着读的问题，以及一句阅读动作。证据、读者视角和调试信息默认折叠；窗口内也保留一个次级“本章判断”按钮。扩展图标 badge 只显示短状态：`…` 表示生成中，`OK` 表示完成，`!` 表示失败。
...
5. 摘要窗口流式显示阅读判断，折叠证据区展示官方 Skill 信号、读者视角和调试信息；扩展图标 badge 同步显示生成、完成或失败状态。

LLM 返回的阅读判断会包含精读/快读/跳读建议、掌握价值分、接下来最需要掌握的内容、追问问题、一句阅读动作，以及折叠证据。

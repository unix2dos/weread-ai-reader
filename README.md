...
> 不止于 AI 摘要。它帮你判断本章能带走什么收获，带着问题阅读，分辨何处速读、何处精读，把阅读的主动权还给你。
...
- 生成章节级阅读判断：精读/快读/跳读建议、收获价值分、能带走的收获、带着读的问题、证据片段和阅读动作。
- 切换章节后自动生成阅读判断；同章内正文采集增长时只更新覆盖率，避免反复打断阅读。
- 使用独立摘要窗口展示 AI 结果，阅读页内不注入可见面板、不改变正文布局。
- 将 WeRead API Key 与 LLM API Key 保留在本地 Agent 服务器，Chrome 扩展只保存服务器地址和 `clientToken`。

## 体验特性

- **辅助阅读，不替代阅读**：输出的是本章读法建议、带着读的问题和可追溯证据片段，不是整章正文复刻。
- **自动但不打扰**：翻到新章节后自动生成；同章内自然阅读带来的正文增量只更新采集状态，用户需要时再手动刷新判断。
- **伴随而不侵入**：结果展示在独立摘要窗口里，不覆盖、不压缩、不改写微信读书正文区域。
- **本地密钥边界**：WeRead API Key 和 LLM API Key 只放在本地 Agent 服务器；如果使用第三方 LLM，章节正文快照和信号面板会发送给你配置的 LLM 服务。

## 内容与版权边界

这个项目只面向你已经有权限阅读的微信读书网页版页面，定位是个人阅读辅助，不是正文抓取、导出或分发工具。

- 不绕过微信读书的登录、付费、会员或版权访问控制。
- 不自动滚动、翻页、跳章节，也不尝试抓取浏览器尚未展示的正文。
- 不提供整本书导出、批量下载、分享正文或重发布内容的能力。
- 调试信息默认只展示正文长度、hash 和截断预览，避免把同一份正文在界面里重复铺开。
- 官方 WeRead Skill 只用于获取章节目录、热门划线、划线评论和公开书评等阅读信号；当前章节正文来自浏览器已经渲染给当前用户的阅读现场。
...
收获价值分由服务端按三个维度派生，不采用模型自由输出的总分：

- 可带走收获：45%
- 理解杠杆：35%
- 投入回报：20%

热门划线和评论是读者注意力信号，不会直接抬高收获价值；只有清晰概念、方法、重要区分、因果链、反证、适用边界或后文枢纽才会提高分数。正文覆盖或公共信号不足时，摘要窗口会显示短的依据偏弱提示。

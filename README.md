...
AI 摘要窗口展示极简首屏：精读/快读/跳读结论、掌握价值总分、信息密度/结构关键/跳读风险三维分、最多 3 个最需要掌握点、最多 2 个带着读的问题、一句阅读动作，以及 Agent 归纳出的读者视角、理由和重点段落。头部采集上下文展示书名、章节名、正文采集字数、覆盖率和采集方式；当前章节的 WeRead 原始信号放在默认展开的“阅读信号”框里；“整本书评价背景”和“调试”默认折叠，其中“调试”只包含请求摘要和完整请求。窗口内也保留一个次级“本章判断”按钮。扩展图标 badge 只显示短状态：`…` 表示生成中，`OK` 表示完成，`!` 表示失败。

...
| **本章阅读价值判断** | 结合整本书评价、本章热门划线、划线评论和章节正文快照，判断当前章节接下来应如何分配注意力、为什么这样读、重点段落在哪里以及读者共识或争议是什么 |
| **实时 AI 反馈** | 用户翻章或请求分析后，围绕本章阅读价值判断生成的阅读辅助结果 |
| **阅读判断** | 主视图中的实时 AI 反馈，面向用户展示本章建议读法、最需要掌握什么、追问问题、读者视角、关键理由、重点段落和下一步阅读动作；它用于分配注意力，不用于评价章节质量；章节变化时自动生成，同章内采集增长时不自动重跑 |
...
| **掌握价值分** | 阅读判断中的 0-100 综合分，表示本章对接下来理解全书或推进阅读最值得投入注意力的程度；它不是文学质量分或作者水平分；只有达到高门槛时才应支撑全章精读结论 |
| **内容增量分** | 掌握价值分的子维度，衡量本章相对前后文新增了多少需要记住、区分或迁移的概念、事实、机制或论证 |
| **结构关键性分** | 掌握价值分的子维度，衡量本章对全书论证、故事推进或概念体系的承上启下作用 |
| **精读必要性分** | 掌握价值分的子维度，衡量本章是否需要整章慢读，还是只需局部精读关键段落后快读其余部分 |
...
| **追问问题** | 阅读判断中给用户的 1-2 个带着阅读的问题，用来提示用户接下来应向作者追问什么；它不包含答案，也不模拟作者对话 |
...
| **阅读信号** | AI 摘要窗口中独立于 Agent 分析的当前章节 WeRead 公共信号区，包括本章热门划线、按划线 range 归属到对应划线下方的评论和信号告警；默认展开，但可以折叠 |
| **整本书评价背景** | AI 摘要窗口中独立展示的全书公开书评背景区，用来补充全书口碑和主题预期；默认折叠，避免压过当前章节判断 |
| **信号面板** | Agent 服务器返回给采集端的 Skill 信号结构，是阅读信号的原始数据来源，不等同于 UI 上的阅读判断 |
| **极简首屏** | AI 摘要窗口默认展示的高价值摘要层，包括头部采集上下文、阅读判断、默认展开的阅读信号、默认折叠的整本书评价背景和默认折叠的调试区；调试区只放请求摘要和完整请求 |
...
2. **掌握价值分**：展示 0-100 综合分，并拆分内容增量分、结构关键性分、精读必要性分。
3. **最需要掌握**：给出 1-3 条接下来最需要掌握的概念、区分或结构。
4. **追问问题**：给出 1-2 个用户应带着阅读的问题，不生成答案。
5. **阅读动作**：用一句话建议用户接下来整章精读、局部精读、如何快读或是否跳读；阅读动作必须服从全章级读法。
6. **读者视角**：展示 Agent 从评论和书评中归纳出的共识、争议、误读或补充。
7. **理由**：给出 1-2 条 Agent 对本章阅读价值判断的依据。
8. **重点段落**：给出 1-3 条 Agent 认为值得关注的热门划线或已采集正文片段。
...
event: complete
data: {"readingJudgement":{"recommendation":"deep_read","masteryScore":{"overall":83,"contentGain":82,"structuralImportance":90,"deepReadNecessity":75},"nextMustKnow":[],"reasons":[],"keyPassages":[],"questionsForAuthor":[],"readerPerspective":"","readingAdvice":""},"judgement":{"conclusion":"worth_deep_read","reasons":[],"keyPassages":[],"readerPerspective":"","readingAction":""}}

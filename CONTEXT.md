...
| **阅读判断** | 主视图中的实时 AI 反馈，面向用户展示本章是否值得精读、最需要掌握什么、追问问题、读者视角、关键理由、重点段落和下一步阅读动作；章节变化时自动生成，同章内采集增长时不自动重跑 |
...
| **掌握价值分** | 阅读判断中的 0-100 综合分，表示本章对接下来理解全书或推进阅读最值得投入注意力的程度；它不是文学质量分或作者水平分；只有达到高门槛时才应支撑“值得精读”结论；它由信息密度分、结构关键性分和可跳读风险分按固定权重派生，不由模型自由给出 |
...
1. **结论**：本章值得精读、可快读或可跳读。
2. **掌握价值分**：展示 0-100 综合分，并拆分信息密度分、结构关键性分、可跳读风险分；综合分由服务端固定权重派生。
3. **最需要掌握**：给出 1-3 条接下来最需要掌握的概念、区分或结构。
4. **追问问题**：给出 1-2 个用户应带着阅读的问题，不生成答案。
5. **阅读动作**：用一句话建议用户接下来精读哪部分、如何快读或是否跳读。
...
event: complete
data: {"readingJudgement":{"recommendation":"deep_read","masteryScore":{"overall":83,"informationDensity":82,"structuralImportance":90,"skipRisk":75},"nextMustKnow":[],"reasons":[],"keyPassages":[],"questionsForAuthor":[],"readerPerspective":"","readingAdvice":""},"judgement":{"conclusion":"worth_deep_read","reasons":[],"keyPassages":[],"readerPerspective":"","readingAction":""}}

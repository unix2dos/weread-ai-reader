...
| **白话阅读判断** | 阅读判断首屏文案的写法约束：读出声要像给朋友说话，术语先翻成人话再点名，短句，一句一事，问题只问一个卡点；不能堆“核心、方法、精神、路线、边界”等抽象词 |
| **带着读的问题** | 阅读判断中给用户的 1-2 个读前验证问题，用来检查本章核心收获是否被读懂；每个问题只问一个卡点，短句，不包含答案，也不模拟作者对话 |
...
| **阅读信号** | AI 摘要窗口中独立于 Agent 分析的当前章节 WeRead 公共信号区，包括本章热门划线、按划线 range 归属到对应划线下方的评论和信号告警；默认展开，直接展示少量高信号划线和对应评论，不再重复放一份“更多阅读信号” |
...
3. **能带走的收获**：给出 1-3 条读完本章最可能看懂什么；专业词先翻成人话，再顺带点名。
4. **带着读的问题**：给出 1-2 个读前验证问题，必须回连到核心收获或证据；每个问题只问一个卡点，不生成答案。
5. **阅读动作**：用一句话建议用户接下来整章精读、局部精读、如何快读或是否跳读；阅读动作必须服从全章级读法，并使用“先读、只看、跳过、回看”等直接动词。

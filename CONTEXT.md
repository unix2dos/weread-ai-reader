...
| **采集端** | 运行在微信读书网页版里的 Chrome 扩展，负责识别当前阅读现场并抓取正文；油猴脚本只作为 DOM 抓取诊断工具 |
...
| **信号面板** | 翻章后优先展示的非 LLM 结果，包括本章热门划线、划线评论摘要和整本书评价背景 |
| **短判断** | 翻章后自动流式生成的简短本章阅读价值判断，可缓存 |
...
3. **整本书公开评价**：背景信号，用于理解全书口碑和阅读预期，不能压过当前章判断。
4. **个人划线/个人想法**：个性化信号，只有在用户授权并配置后加入。

## 短判断结构

1. **结论**：本章值得精读、可快读或可跳读。
2. **理由**：基于章节正文快照、热门划线和划线评论给出 2-3 条证据。
3. **重点段落**：列出 3-5 条热门划线或正文片段。
4. **读者视角**：总结评论中的共识、争议、误读或补充。
5. **阅读动作**：建议用户接下来精读哪部分、带着什么问题读。
...
**决策**：最终产品和 V1 验证形态都是 Chrome 扩展。油猴脚本只保留为 DOM 抓取诊断工具，不作为端到端产品验证路线。
...
- 油猴验证再迁移会产生一次性代码和重复调试成本

### 触发模式：信号先到 + 短判断自动 + 长分析手动

**决策**：翻章时先自动返回信号面板，再自动流式生成短判断；长版深度分析按需手动触发。

**理由**：
- 信号面板不调 LLM，尽快显示当前章热门划线、划线评论和整本书评价背景
- 短判断让实时 AI 跟读成立，避免每章都只停留在数据罗列
- 长版深度分析消耗更多 token，按需触发控制成本
- 用户体验最自然：先看到读者信号，再看到 AI 判断，感兴趣时再深入

### 架构：浏览器采集端 + Agent 服务器（V1）

**决策**：V1 需要独立 Agent 服务器。Chrome 扩展作为采集端，负责抓取微信读书网页正文快照；Agent 服务器负责调用官方 WeRead Skill 和 LLM，生成实时 AI 反馈。油猴脚本只作为 DOM 抓取诊断工具。

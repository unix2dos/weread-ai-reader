...
| **阅读信号** | AI 摘要窗口中独立于 Agent 分析的 WeRead 原始信号区，包括正文采集覆盖、本章热门划线、按划线 range 归属到对应划线下方的评论，以及整本书评价背景；默认展开，但可以折叠 |
| **信号面板** | Agent 服务器返回给采集端的 Skill 信号结构，是阅读信号的原始数据来源，不等同于 UI 上的阅读判断 |
| **极简首屏** | AI 摘要窗口默认展示的高价值摘要层，包括阅读判断、默认展开的阅读信号和默认折叠的调试区；调试区只放请求摘要和完整请求 |
